...
def compute_rewrite_quality_counterfact(
    ...
    probs, targets_correct = test_batch_prediction(
        ...
        which_correct=list(chain(*which_correct)),
...
def test_batch_prediction(
    ...
    prefix_lens = [len(n) for n in mt.tokenizer(prefixes)["input_ids"]]
    prompt_tok = mt.tokenizer(
        [
            f"{prefix} {suffix}"
    ...
    a_tok, b_tok = (
        mt.tokenizer(f" {n}")["input_ids"] for n in [target_new, target_true]
    )
    choice_a_len, choice_b_len = (len(n) for n in [a_tok, b_tok])

    with torch.no_grad():
        output = mt(**prompt_tok)
...
def test_generation(
    ...
    if len(essence_texts) > 0:
        ppl = perplexity(mt=mt, text=" ".join(essence_texts), max_input_length=100)
