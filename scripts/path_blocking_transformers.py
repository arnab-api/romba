...
def path_ablation(
    ...
    assert alt_subj_patching == True, "only alt_subj_patching is supported for now"
    ...
    def patch_rep(repr, layer):
        ...
        if layer not in residual_patch_spec and layer not in block_unpatch_spec:
            # no intervention needed. just continue
            return repr

        # If this layer is in the patch_spec, restore the uncorrupted hidden state
        # for selected tokens from the first run
        h = untuple(repr)
        if layer in residual_patch_spec:
            for t in residual_patch_spec[layer]:
                h[1:, t] = untuple(first_run_residual_activations[layer].output)[0, t]

        if layer in block_unpatch_spec:
            for t in block_unpatch_spec[layer]:
                # print(f">>>>> corrupting {layer} at token {t}")
                h[1:, t] = untuple(first_run_residual_activations[layer].output)[1, t]
    ...
    states_to_cache = [embed_layername] + list(residual_patch_spec.keys())
    if is_mamba_variant(mt) == False:
        states_to_cache += list(block_unpatch_spec.keys())
    with torch.inference_mode(), baukit.TraceDict(
        mt.model, states_to_cache, edit_output=None  # No intervention on the clean run
    ) as td:
        mt(**inp)
...
def trace_important_states_with_ablation(
    ...
    ablate_transformer_module: Optional[Literal["mlp", "attn"]] = None,
    alt_subj_patching: bool = False,
):
    assert ablate_mambahook is None or ablate_transformer_module is None
    ...
    for tnum in token_range:
        ...
        if ablate_transformer_module is not None:
            # print("?????", ablate_transformer_module)
            if ablate_transformer_module == "mlp":
                module_name_format = mt.mlp_module_name_format
            elif ablate_transformer_module == "attn":
                module_name_format = mt.attn_module_name_format
            else:
                raise ValueError(
                    f"Invalid transformer module name => {ablate_transformer_module}"
                )

            block_states_to_unpatch = [
                (tnum, module_name_format.format(layer))
                for layer in range(0, mt.n_layer)
            ]
        # print(block_states_to_unpatch)
...
def calculate_hidden_flow_with_ablation(
    ...
    # print(" >>>>", ablate_transformer_module)
    assert (
        ablate_mambahook is None or ablate_transformer_module is None
    ), "Only one of ablate_mambahook or ablate_transformer_module can be set"
    ...
    differences = trace_important_states_with_ablation(
        ...
        ablate_transformer_module=ablate_transformer_module,
        alt_subj_patching=alt_subject is not None,
    )
    ...
    indirect_effect = dict(
        ...
        kind=ablate_mambahook or ablate_transformer_module,
    )
...
def run_ablation_experiment(
    ...
    results_dir = os.path.join(
        RESULTS_DIR, "ablation", model_path.lower().split("/")[-1]
    )
        ...
        for setting in ["subject_last", "prompt_last"]:
            ...
            for sample in tqdm(samples):
                ...
                indirect_effect = calculate_hidden_flow_with_ablation(
                    ...
                    ablate_transformer_module=None,
                    **common_kwargs,
                )

                indirect_effect_attn_severed = calculate_hidden_flow_with_ablation(
                    subject=sample.subject,
                    alt_subject=alt_sample.subject,
                    ablate_transformer_module="attn",
                    **common_kwargs,
                )
                ...
                attn_severed_recovery = (
                    indirect_effect_attn_severed["scores"] - low_score
                ) / (high_score - low_score)
                ...
                ablation_results.trials.append(
                    AblationResult(
                        ...
                        attn_severed=attn_severed_recovery.tolist(),
