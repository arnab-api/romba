...
class Relation(DataClassJsonMixin, Dataset):
    ...
    _few_shot_samples: list[Sample] = field(default_factory=list)
    ...
    def without(self, sample: Sample) -> "Relation":
...
def load_relation(file: PathLike) -> Relation:
    ...
    return Relation(
        ...
        samples=[Sample.from_dict(sample) for sample in relation_dict["samples"]],
